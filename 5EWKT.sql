...
SELECT ST_Area(ST_Buffer(ST_ShortestLine(obiekt3.geometry, obiekt4.geometry), 5))
	FROM obiekty obiekt3, obiekty obiekt4
...
INSERT INTO obiekty(id, name, geometry)  VALUES
	(8, 'obiekt7', ST_Collect((SELECT geometry FROM obiekty WHERE name = 'obiekt3'), (SELECT geometry FROM obiekty WHERE name = 'obiekt4')))
...
SELECT ST_Area(ST_Buffer(geometry, 5))
	FROM obiekty WHERE NOT ST_HasArc(geometry)
-- zsumowanie wynikow
SELECT SUM(ST_Area(ST_Buffer(geometry, 5)))
	FROM obiekty WHERE NOT ST_HasArc(geometry)

-- ST_HasArc zwraca prawde jesli geometria zawiera "łuki" w stringach